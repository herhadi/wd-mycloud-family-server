...
| Network | `eth0`, `192.168.11.125/24` (observation at verification time) |
...
At the latest recorded baseline check:

- RAM: 226 MB total
- Approximately 104 MB free was reported by the kernel-level view.
- Approximately 197 MB was reported in the `-/+ buffers/cache` view.
- Swap: 511 MB available, approximately 40 MB used at that observation.

Memory numbers are point-in-time observations rather than capacity guarantees. Syncthing workload, SMB clients, and future services can change them materially.

## Storage state

`/data` is an ext4 filesystem on `/dev/sda4` with approximately 2.7 TB capacity. At the latest recorded check it used approximately 13 GB.
...
- Do not expose the Syncthing GUI directly to the Internet.
...
- The IP address above is a recorded observation, not a static configuration guarantee.
